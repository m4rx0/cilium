	...
	go func() {
		for {
			...
			case <-e.client.Ctx().Done():
				initLockSucceeded <- fmt.Errorf("client context ended")
...
func (e *etcdClient) waitForInitialSession(ctx context.Context) error {
	...
	case <-ctx.Done():
		return fmt.Errorf("interrupt while waiting for initial session to be established: %s", ctx.Err())
	}
